...
export const usePlayerStore = defineStore('player', () => {
  ...
  function cultivate() {
    ...
    if (Math.random() < 0.02) {
      ...
      return { type: 'event' as const, text: evt.text }
    }

    return { type: 'cultivate' as const, gain }
  }
  ...
  function ageUp() {
    ...
    if (remaining <= 10) {
      return { dead: false, warning: true, remaining }
    }
    return { dead: false, warning: false, remaining }
  }
  ...
  function useMp(amount: number) {
    if (mp.value < amount) return false
    mp.value -= amount
    return true
  }

  function addItem(itemName: string, count: number = 1) {
    items.value[itemName] = (items.value[itemName] || 0) + count
  }

  function removeItem(itemName: string, count: number = 1) {
    if (!items.value[itemName] || items.value[itemName] < count) return false
    items.value[itemName] -= count
    if (items.value[itemName] <= 0) delete items.value[itemName]
    return true
  }

  function useItem(itemName: string, itemDef: { effect: ((p: any) => void) | null }) {
    if (!items.value[itemName] || items.value[itemName] <= 0) return false
    if (itemDef.effect) {
      const proxy = {
        get hp() { return hp.value }, set hp(v: number) { hp.value = v },
        get maxHp() { return maxHp.value }, set maxHp(v: number) { maxHp.value = v },
